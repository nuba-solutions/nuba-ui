...
const SidebarOne = ({isSidebarOpen, isSidebarCompact, setIsSidebarOpen} : any) => {
    ...
    return (
        <aside className={`fixed top-[70px] z-40 ${isSidebarOpen ? 'left-0' : '-left-full -translate-x-full'} lg:sticky lg:translate-x-0 lg:left-0 ${isSidebarCompact? 'w-[70px]' : 'w-[250px]'} h-[calc(100vh-70px)] pt-3 transition-[width] bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-gray-700`} aria-label="Sidebar">
            <div className={`h-full ${isSidebarCompact? 'w-[70px]' : 'w-[250px]'} px-3 pb-4 overflow-y-auto overflow-x-hidden scrollbar-hide bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-gray-700 transition-[width]`}>
                <ul className="space-y-2 font-medium">
                    ...
                    <SidebarListItem name='Account' iconLeft={<IoPerson/>} compact={isSidebarCompact} dropdown>
                        <hr className={`${isSidebarCompact ? 'hidden' : 'visible'} h-px bg-white border-0 dark:bg-gray-600`}></hr>
                ...
                <ul className="space-y-2 font-medium">
                    <SidebarListItem name='Energy' iconLeft={<HiLightningBolt/>} compact={isSidebarCompact} dropdown>
                        ...
                        <ul className={`bg-gray-100 dark:bg-gray-700 rounded-lg shadow-lg ${isSidebarCompact ? 'rounded-tl-none absolute z-10 min-w-[200px] shadow-xl' : 'rounded-t-none'} overflow-clip`}>
                            ...
                            <DropdownListItem name='Vitamin' onClick={() => setIsSidebarOpen(false)}/>
                        </ul>
                    </SidebarListItem>
