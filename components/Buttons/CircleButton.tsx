...
const CircleButton: React.FC<CircleButtonProps> = ({visual, variant, link, children, classes, center, right, left, sz, disabled, onCLick }) => {
    let tp: string
    ...
    if (link) {
        return (
            <Link
                href={link}
                className={`rounded-full flex items-center justify-center focus-visible:outline focus-visible:outline-4 ${classes} ${sz} ${tp} ${center ? 'mx-auto' : ''} ${right ? 'ml-auto' : ''} ${left ? 'mr-auto' : ''} ${disabled ? 'opacity-50 pointer-events-none select-none' : ''}`}
            >
                {children}
            </Link>
        )
